...
使用方法：
1. 在 flutter_codegen 目录内运行：python test.py
2. 从项目根目录运行：python -m flutter_codegen.test
"""
import sys
import os
import shutil
...
# 添加父目录到路径（如果在 flutter_codegen 目录内运行）
if Path(__file__).parent.name == 'flutter_codegen':
...
# Flutter 代码生成配置
BASE_PACKAGE = "com.example.api"  # Flutter 包名
OUTPUT_DIR = "output/flutter_output"     # 输出目录（默认统一到 output 文件夹）

# 输出目录配置
# None: 使用默认目录（flutter_output）
# 字符串: 导出到指定目录，支持 {item_id} 占位符
# 例如: "output/flutter_{item_id}" 或 "output/flutter_code"
...
def get_output_dir(api_tree, output_dir_config=None, auto_generate=False):
    ...
    if not auto_generate:
        # 交互式询问
        try:
            generate = input(f"  是否生成 Flutter 代码到 '{output_dir}' ？(y/n, 默认n): ").strip().lower()
            if generate != 'y':
                print("  - 跳过生成")
                return None
        except EOFError:
...
def main():
    ...
    try:
        ...
        # 步骤5: 生成 Flutter 代码（使用版本控制，不再删除旧目录）
        print("[步骤 5] 生成 Flutter 代码...")
        print("  提示: 正在生成 Dio Service、实体类和配置文件...")
        ...
        if generated_files.get("models"):
            print(f"\n  [实体类] ({len(generated_files['models'])} 个文件):")
            for file_path in generated_files["models"]:
                file_name = Path(file_path).name
                print(f"    - {file_name}")
        
        if generated_files.get("repositories"):
            print(f"\n  [Repository] ({len(generated_files['repositories'])} 个文件):")
            for file_path in generated_files["repositories"]:
                file_name = Path(file_path).name
                print(f"    - {file_name}")
        ...
        print(f"  2. 将生成的代码复制到你的 Flutter 项目中")
        print(f"  3. 添加依赖配置到 pubspec.yaml")
        print(f"  4. 运行 flutter pub get")
        print(f"  5. 运行 flutter pub run build_runner build 生成 .g.dart 文件")
        print(f"  6. 根据实际 API 基础 URL 修改 dio_config.dart")
        print()
